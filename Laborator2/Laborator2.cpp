...
void Laborator2::Init()
{
	...
	// Create a mesh box using custom data
	{
		...
		vector<unsigned short> indicesTetra =
		{
			0, 1, 2,	// indices for first triangle
			0, 2, 3,
			1, 2, 3,
		...
		// Create a new mesh from buffer data
		Mesh* tetra = CreateMesh("tetra", verticesTetra, indicesTetra);
	
		vector<VertexFormat> verticesSqare
		{
			VertexFormat(glm::vec3(0, 0,  0), glm::vec3(0, 1, 1)),
			VertexFormat(glm::vec3(1, 0,  0), glm::vec3(0, 1, 1)),
			VertexFormat(glm::vec3(1, 1,  0), glm::vec3(0, 1, 1)),
		...
		vector<unsigned short> indicesSqare =
		{
		...
		// Create a new mesh from buffer data
		Mesh* sqare = CreateMesh("sqare", verticesSqare, indicesSqare);
	}
}
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["tetra"], shaders["VertexColor"], glm::vec3(1.5f, -0.5f, 0), glm::vec3(0.75f));

	RenderMesh(meshes["sqare"], shaders["VertexColor"], glm::vec3(0.5f, 0.5f, 0), glm::vec3(0.75f));
