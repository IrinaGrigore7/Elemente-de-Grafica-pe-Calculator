...
Mesh* ProgressBar::Create(std::string name, glm::vec3 color, float width, float height, float maximum, float minimum)
{
...
void ProgressBar::CalculateCircleCollision()
{
	int x = position.x - width / 2;
	int y = position.y - width / 2;
	int w = width;
	int h = width;
	boundingBox = glm::vec4(x, y, w, h);
}
